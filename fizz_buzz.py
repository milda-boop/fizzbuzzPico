...
LCD.text('Press Y to start', 10, 10, colour(0,0,255))
#printstring('Press Y to start',43,10,3,True,False,colour(0,0,255))
LCD.text('Press A to Fizz', 10, 20, colour(255,255,0))
LCD.text('Press B to Buzz', 10, 30, colour(255,255,0))
LCD.text('Press X to FizzBuzz', 10, 40, colour(255,255,0))
LCD.text("Highscore: ", 10, 200, colour(255,0,0))
#LCD.text(" Halt program", 30, 220, colour(255,0,0))
LCD.show()

running = True # Loop control
gameRunning = False # GameLoop control
highscore = 0;
printstring(str(highscore),95,195,2,True,False,colour(255,0,0))
# =========== Main loop ===============
while(running):
    if keyA.value() == 0:
    ...
    printstring('0',105,105,3,True,False,LCD.white)
    count = 1
    LCD.show()
    if (keyA.value() == 0) and (keyB.value() == 0): # Halt looping?
        running = False
    ...
    while(gameRunning):
        ...
        if(count>highscore):
            printstring(str(highscore),95,195,2,True,False,colour(40,40,40))
            printstring(str(count),95,195,2,True,False,colour(255,0,0))
            highscore = count
        printstring(str(0),105,105,3,True,False,colour(40,40,40))
        printstring(str(count),105,105,3,True,False,LCD.white)
        LCD.show()
        correctAnswer = False
        awaitAnswer = True
        ...
        # =========== Inner Game loop ===============
        while(awaitAnswer and utime.ticks_diff(deadline,utime.ticks_ms())>0):
            #print("Entering inner game loop")
            if(keyY.value() == 0):
                LCD.fill_rect(200,195,30,30,colour(255,180,50)) # Orange
                gameRunning = False
                awaitAnswer = False
        ...
        if((not correctAnswer) and (count%3==0 or count%5==0)):
                gameRunning = False
        
        printstring(str(count),105,105,3,True,False,colour(40,40,40))
        LCD.show()
        count = count+1
